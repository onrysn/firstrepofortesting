package com.bixolon.PrinterControl;
...
public class BixolonPrinter implements ErrorListener, OutputCompleteListener, StatusUpdateListener, DirectIOListener, DataListener
{
    ...
    public boolean cutPaper()
    {
        try {
            ...
            posPrinter.cutPaper(90);    // Partial Cut
            // posPrinter.cutPaper(100);            // Full Cut
        } catch (JposException e) {
            e.printStackTrace();
    ...
    private byte[] ConvertHexaToInteger(byte[] szHexa, int nSize)
    {
        ...
        while ( j < nSize )
        {
            ch1 = ConvertINT(szHexa[j]);	bData  = (byte)(ch1 << 4);	j++;
            ch2 = ConvertINT(szHexa[j]);	bData += (ch2 & 0x0f);		j++;
            hex.put((byte)bData);
        }
    ...
    public void errorOccurred(ErrorEvent errorEvent) {
        ...
        //app.ShowMessage(null, "errorOccurred", "");
    	/*
        Fragment fm = MainActivity.getVisibleFragment();
        if(fm != null) {
