...
public class SQLDroidConnection implements Connection {
	...
	/**
	 * A map from a connection to a SQLiteDatabase instance. Used to track the
	 * use of each instance, and close the database when last conneciton is
	 * closed.
	 */
	 ...
	 * constructor: SQLException(Throwable theCause) otherwise false. API levels
	 * 9 or greater have this constructor. If the value is positive and less
	 * than 9 then the SQLException does not have the constructor. If the value
	 * is &lt; 0 then the capabilities of SQLException have not been determined.
	 */
	 ...
	 *
	 * @param url
	 *            the URL string, typically something like
	 *            "jdbc:sqlite:/data/data/your-package/databasefilename" so for
	 *            example:
	 *            "jdbc:sqlite:/data/data/org.sqldroid.examples/databases/sqlite.db"
	 * @param info
	 *            Properties object with options. Supported options are
	 *            "timeout", "retry", and "shared".
	 */
	public SQLDroidConnection(String url, Properties info) throws SQLException {
		Log.v("SQLDroidConnection: " + Thread.currentThread().getId() + " \"" + Thread.currentThread().getName() + "\" "
				+ this);
		...
		if (url.startsWith(SQLDroidDriver.xerialPrefix)) {
			...
			// So we could check that the url starts with
			// SQLDroidDriver.sqldroidPrefix
			// but if it doesn't there's nothing we can do (no Exception is
			// specified)
			// so it has to be assumed that the URL is valid when passed to this
			// method.
			dbQname = url.substring(SQLDroidDriver.sqldroidPrefix.length());
		}
		long timeout = 0; // default to no retries to be consistent with other
							// JDBC implemenations.
		long retryInterval = 50; // this was 1000 in the original code. 1 second
									// is too long for each loop.
		...
		if (queryPart > 0) {
			...
			while (options.length() > 0) {
				...
				try {
					...
					// print and ignore
					Log.e("Error Parsing URL \"" + url + "\" Timeout String \"" + optionValueString
							+ "\" is not a valid long", nfe);
				}
		...
		if (info != null) {
			if (info.getProperty(SQLDroidDriver.DATABASE_FLAGS) != null) {
				...
				try {
					...
					Log.e("Error Parsing DatabaseFlags \"" + info.getProperty(SQLDroidDriver.DATABASE_FLAGS)
							+ " not a number ", nfe);
				}
				...
				try {
					...
					Log.e("Error Parsing DatabaseFlags \"" + info.getProperty(SQLDroidDriver.ADDITONAL_DATABASE_FLAGS)
							+ " not a number ", nfe);
				}
			}
		}
		synchronized (dbMap) {
			...
			if (sqlitedb == null) {
				Log.i("SQLDroidConnection: " + Thread.currentThread().getId() + " \"" + Thread.currentThread().getName()
						+ "\" " + this + " Opening new database: " + dbQname);
	...
	private void ensureDbFileCreation(String dbQname) throws SQLException {
		...
		if (dbFile.isDirectory()) {
			...
			throw new SQLException(
					"Can't create " + dbFile + " - it because " + dbFile.getParent() + " exists as a regular file");
	...
	/**
	 * This will create and return an exception. For API levels less than 9 this
	 * will return a SQLDroidSQLException, for later APIs it will return a
	 * SQLException.
	 */
	public static SQLException chainException(android.database.SQLException sqlException) {
		if (sqlThrowable < 0 || sqlThrowable >= 9) {
			try {
				...
				// creating by reflection is significantly slower, but since
				// Exceptions should be unusual
		...
		// if the code above worked correctly, then the exception will have been
		// returned. Otherwise, we need
		// to go through this clause and create a SQLDroidSQLException
		try {
			// avoid a direct reference to the sqldroidSQLException so that app
			// > API level 9 do not need that class.
			final Constructor<?> c = SQLDroidConnection.class.getClassLoader()
					.loadClass("org.sqldroid.SQLDroidSQLException")
					.getDeclaredConstructor(new Class[] { android.database.SQLException.class });
			// SQLDroidSQLException is an instance of (direct subclass of)
			// SQLException, so the cast below is correct although
	...
	public void close() throws SQLException {
		Log.v("SQLDroidConnection.close(): " + Thread.currentThread().getId() + " \"" + Thread.currentThread().getName()
				+ "\" " + this);
		if (sqlitedb != null) {
			synchronized (dbMap) {
				clientMap.remove(this);
				if (!clientMap.containsValue(sqlitedb)) {
					Log.i("SQLDroidConnection.close(): " + Thread.currentThread().getId() + " \""
							+ Thread.currentThread().getName() + "\" " + this
							+ " Closing the database since since last connection was closed.");
					setAutoCommit(true);
					sqlitedb.close();
			...
			Log.e("SQLDroidConnection.close(): " + Thread.currentThread().getId() + " \""
					+ Thread.currentThread().getName() + "\" " + this + " Duplicate close!");
		}
	}
	...
	public void commit() throws SQLException {
		...
		Log.d("END TRANSACTION  (commit) " + Thread.currentThread().getId() + " \"" + Thread.currentThread().getName()
				+ "\" " + this);
		sqlitedb.endTransaction();
		
		Log.d("BEGIN TRANSACTION (after commit) " +
	...
	public SQLWarning getWarnings() throws SQLException {
		// TODO: Is this a sufficient implementation? (If so, delete comment and
		// logging)
	...
	public boolean isClosed() throws SQLException {
		...
		return sqlitedb == null || sqlitedb.getSqliteDatabase() == null || !sqlitedb.getSqliteDatabase().isOpen();
	}
	...
	public void releaseSavepoint(Savepoint savepoint) throws SQLException {
		// TODO: Implemented in Xerial as db.exec(String.format("RELEASE
		// SAVEPOINT %s", savepoint.getSavepointName()));
	...
	public void rollback() throws SQLException {
		...
		Log.d("END TRANSACTION (rollback) " + Thread.currentThread().getId() + " \"" + Thread.currentThread().getName()
				+ "\" " + this);
		sqlitedb.endTransaction();
		Log.d("BEGIN TRANSACTION (after rollback) " + Thread.currentThread().getId() + " \""
				+ Thread.currentThread().getName() + "\" " + this);
	...
	public void rollback(Savepoint savepoint) throws SQLException {
		// TODO: Implemented in Xerial as db.exec(String.format("ROLLBACK TO
		// SAVEPOINT %s", savepoint.getSavepointName()));
	...
	public void setAutoCommit(boolean autoCommit) throws SQLException {
		...
		if (autoCommit) {
			...
			if (sqlitedb.inTransaction()) { // to be on safe side.
				sqlitedb.setTransactionSuccessful();
				Log.d("END TRANSACTION (autocommit on) " + Thread.currentThread().getId() + " \""
						+ Thread.currentThread().getName() + "\" " + this);
				sqlitedb.endTransaction();
			}
			
		} else {
			Log.d("BEGIN TRANSACTION (autocommit off) " + Thread.currentThread().getId() + " \""
					+ Thread.currentThread().getName() + "\" " + this);
			sqlitedb.beginTransaction();
			//this.autoCommit = autoCommit;
	...
	public Savepoint setSavepoint() throws SQLException {
		// TODO: In Xerial: db.exec(String.format("SAVEPOINT %s",
		// sp.getSavepointName()))
	...
	public Blob createBlob() throws SQLException {
		// TODO: Can return new SQLDroidBlob(new byte[0]) once setBytes is
		// implemented
	...
	public Properties getClientInfo() throws SQLException {
		// TODO Evaluate if this is a sufficient implementation (if so, remove
		// this comment)
	 ...
	 *         deleted by the most recently completed INSERT, DELETE, or UPDATE
	 *         statement.
	 *
	 * @throws SQLException
	 */
	public int changedRowsCount() {
		...
		try {
			...
			if (changedRowsCountResultSet != null && changedRowsCountResultSet.first()) {
				...
				// System.out.println("In SQLDroidConnection.changedRowsCount(),
	...
	/**
	 * @return A cached prepare statement for the last row id generated by the
	 *         database when executing an INSERT statement or create a new
	 *         prepare statement and then return that.
	 *
	 * @throws SQLException
	 */
	...
	/**
	 * @return A cached prepare statement for the count of changed rows or
