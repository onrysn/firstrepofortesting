...
public class SQLDroidPreparedStatement implements PreparedStatement {
	...
	/**
	 * True if the sql statement MAY produce a result set. For example, "create"
	 * and "drop" command will produce a "false" value for this.
	 */
	protected boolean potentialResultSet;

	/**
	 * The update count. We don't know this, but need to respond in such a way
	 * that: (from getMoreResults) There are no more results when the following
	 * is true:
	 *
	 ...
	 * then getUpdateCount will return -1. If there is no result set, then,
	 * presumably, <code>execute()</code> was called and we have one result and
	 * so can return something other than -1 on the first call to
	 * getUpdateCount. In this case, the second call to getUpdateCount we should
	 * return -1; We set this to zero on execute() and decrement it on
	 * getUpdateCount. If the value of updateCount is -1 then we just return it
	 * from getUpdateCount.
	 */
	private int updateCount = -1;

	/**
	 * The ResultSet object containing the last row id generated by the database
	 * when executing an INSERT statement.
	 */
	private ResultSet rowIdResultSet = null;

	/**
	 * The value used for autoGeneratedKeys in Connection.prepareStatement.
	 * Defaults to NO_GENERATED_KEYS
	 */
	...
	private void setObj(int n, Object obj) {
		...
		// System.out.println("n = " + n + " size now " + l.size() + " we @ " +
		// n);
		l.set(n, obj);
		// System.out.println("POST set n = " + n + " size now " + l.size() + "
		// we @ " + n);
	}
	...
	public void setSQL(String sql) {
		...
		// examples of a failure on the next line (so isSelect is false and
		// potentialResultSet remains true, are PRAGMA and INSERT (why INSERT?)
		if (!isSelect && (sql.toUpperCase().matches("(?m)(?s)\\s*CREATE.*")
				|| sql.toUpperCase().matches("(?m)(?s)\\s*DROP.*"))) {
	...
	public void clearWarnings() throws SQLException {
		// TODO: Evaluate if implementation is sufficient (if so, delete comment
		// and log)
	...
	public boolean execute() throws SQLException {
		updateCount = -1;
		closeResultSet();
		// problem, a PRAGMA statement (and maybe others) should also cause a
		// result set
	...
	public int executeUpdate() throws SQLException {
		...
		if (autoGeneratedKeys == PreparedStatement.RETURN_GENERATED_KEYS) {
			// Capture the generated rowId immediately after executing the
			// (insert) statement
			rowIdResultSet = sqldroidConnection.getGeneratedRowIdResultSet();
			return 1; // This should be more efficient than
						// sqldroidConnection.changedRowsCount()
		}
		return updateCount;
	}
	...
	public int executeUpdate(String thisSql, int autoGeneratedKeys) throws SQLException {
		// System.out.println("sql in SQLDoidPreparedStatement.executeUpdate()
		// is " + sql);
	...
	public ResultSet getGeneratedKeys() throws SQLException {
		...
		if (rowIdResultSet != null) {
			rowIdResultSet = null; // so the next time it would be returned as
									// null
		}
		return tmp;
	}
	 ...
	 * method should be called only once per result. Returns: the current result
	 * as an update count; -1 if the current result is a ResultSet object or
	 * there are no more results
	 */
	@Override
	public int getUpdateCount() throws SQLException {
		if (updateCount != -1) { // for any successful update/insert, update
									// count will have been set
			// the documentation states that you're only supposed to call this
			// once per result.
			// on subsequent calls, we'll return -1 (which would appear to be
			// the correct return
			int tmp = updateCount;
			updateCount = -1;
			return tmp;
		}
		return updateCount; // if the result was a result set, or this is the
							// second call, then this will be -1
	}
	 ...
	 * @param inputStream
	 *            the input stream from which a byte array will be read and set
	 *            as the value. If inputStream is null this method will throw a
	 *            SQLException
	 * @param length
	 *            a positive non-zero length values
	 * @exception SQLException
	 *                thrown if the length is &lt;= 0, the inputStream is null,
	 *                there is an IOException reading the inputStream or if
	 *                "setBytes" throws a SQLException
	 */
	@Override
	...
	public void setUnicodeStream(int parameterIndex, InputStream theInputStream, int length) throws SQLException {
		setCharacterStream(parameterIndex, new InputStreamReader(theInputStream, UTF_8), length);
	}
	...
	public void setAsciiStream(int parameterIndex, InputStream inputStream, long length) throws SQLException {
		setCharacterStream(parameterIndex, new InputStreamReader(inputStream, US_ASCII), length);
	}

	/**
	 * Read the byte stream and set the object as a byte[]. This is a pass
	 * through to
	...
	/**
	 * This is a pass through to the integer version of the same method. That
	 * is, the long is downcast to an integer. If the length is greater than
	 * Integer.MAX_VALUE this method will throw a SQLException.
	 * 
	 * @see #setBinaryStream(int, InputStream, int)
	 * @exception SQLException
	 *                thrown if length is greater than Integer.MAX_VALUE or if
	...
	public void setBlob(int parameterIndex, InputStream inputStream, long length) throws SQLException {
		// TODO setObject(parameterIndex, new SQLDroidBlob(inputStream,
		// length));
